...
void tensor_pool_destroy(tensor_pool_t *pool) {
    debug("tensor_pool_destroy: pool size=%zu used=%zu space", tensor_pool_size(pool),
...
void *tensor_pool_alloc(tensor_pool_t *pool, size_t size, uint32_t *id) {
    ...
    if (pool->memused + size > pool->memsize) {
        debug("tensor_pool_alloc: memory exhausted, size=%zu, used=%zu, total=%zu", size,
              pool->memused, pool->memsize);
        return NULL;
    }
